...
<body ng-controller="my_ctrl">
	...
	<%-- <c:url var="addAction" value="<%=application.getContextPath() %>/spring-mvc/registration" ></c:url>--%>		
			<form:form action="./spring-mvc/registration" method="POST"
	...
	</select><br> <label>Email:</label>&emsp;&emsp;<span style="color: red;">{{emailMsg}}</span> <br>
	<input type="text" class="form-control" name="register.email" ng-model="email" title="Enter Correct Email eg.jhon@gmail.com" ng-pattern="isValidEmail" id="email" ng-mouseleave="isEmailExists({'email':email})"/><br>
...
<div class="col-md-4">
	...
	<label>Country:</label><br>
	 <select class="form-control" name="country.c_Id" id="c_Id" ng-model="c_Id"
			ng-change="getStateData({'c_id':c_Id})" placeholder="Select">
		<option ng-repeat="x in country_data" ng-selected="x.c_Name == true" value={{x.c_Id}}>{{x.c_Name}}</option>
		</select><br>
		 <label>State:</label><br> 
		<select class="form-control" name="state.s_Id" ng-model="s_Id" id="s_Id"
		ng-change="getCityData({'s_Id':s_Id})">
		<option ng-repeat="x in state_data" value={{x.s_Id}}>{{x.s_Name}}</option>
	...
	<input type="text" class="form-control" name="address.address"
						id="address" /><br>
					<!-- <label>line2:</label><br><input type="text" class="form-control" name="line2" id="line2"/><br> <br>
			 -->
				...
				<input type="file" onchange="loadFile(event)" ng-model="userImage" id="userImage" name="userImage">
...
app.service('state_service',function($http,$q){
	...
	this.getStateData=function(data){
		...
		.then(function(posRes){
			alert("hii "+posRes.data[0].s_Name);
			return posRes.data;
		},function(errRes){
			alert("error");
